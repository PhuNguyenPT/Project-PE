<template>
  ...
  <button @click="goToAdminDashboard" class="btn-back">Back to Admin Dashboard</button>
    ...
    <form @submit.prevent="uploadProduct" enctype="multipart/form-data">
      ...
      <div class="input-field">
        <label for="categoryIds">Category IDs:</label>
        <input type="text" v-model="product.categoryIds" />
      </div>
      <div class="input-field">
    ...
    <form @submit.prevent="updateProduct(productId)" enctype="multipart/form-data">
      ...
      <div class="input-field">
        <label for="categoryIds">Category IDs:</label>
        <input type="text" v-model="updateData.categoryIds" />
      </div>
...
try {
  ...
  console.error('Error uploading product:', error);
    ...
    async updateProduct(productId) {
      ...
      try {
        const response = await fetch(`https://localhost/api/v1/products/update/${productId}`, {
          method: 'PATCH',
          body: formData,
          headers: { 'Authorization': 'Bearer this.getToken()' }
        });
